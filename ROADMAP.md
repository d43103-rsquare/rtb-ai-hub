...
### 멀티 환경 지원 (2026-02-08 완료)

- ✅ **단일 인프라 멀티 환경**: int(개발)/stg(검증)/prd(운영) 논리적 분리
  - 웹훅 환경 라우팅: `?env=stg` 쿼리 파라미터 또는 `X-Env` 헤더
  - BullMQ 큐를 통한 환경 컨텍스트 전파
  - 환경별 MCP 서버 컨테이너 (mcp-jira-int, mcp-jira-stg 등)
  - DB `env` 컬럼 추가 (workflow_executions, webhook_events)
  - 환경별 자격증명 분리 (INT*\*, STG*\* 환경변수)
  - Drizzle ORM 마이그레이션 (0001_add_env_column.sql)
- ✅ **Docker Compose**: 8개 환경별 MCP 컨테이너 추가 (4서비스 × int/stg)
- ✅ **테스트**: 140개 전체 통과 (빌드 + 린트 + 테스트)
...
**완료된 기능**: 단일 인프라에서 int/stg/prd 환경 분리

**성과**:

- ✅ 웹훅 환경 라우팅 (`?env=stg` 또는 `X-Env` 헤더)
- ✅ 환경별 MCP 서버 컨테이너 (8개 추가)
- ✅ DB 환경 컬럼 + Drizzle 마이그레이션
- ✅ 환경별 자격증명 분리 (폴백 지원)
- ✅ 140개 테스트 전체 통과

**영향**:

- 테스트 기간 비용 절감 (단일 인프라로 3환경 운영)
- 환경별 독립적 외부 서비스 연동 가능
- 실제 웹훅 설정 없이 curl로 환경별 테스트 가능

**관련 파일**:

- `packages/shared/src/types.ts` (Environment 타입)
- `packages/shared/src/constants.ts` (MCP_ENDPOINTS_BY_ENV)
- `packages/webhook-listener/src/routes/*.ts` (환경 라우팅)
- `packages/workflow-engine/src/workflows/*.ts` (환경 전파)
- `docker-compose.yml` (환경별 MCP 컨테이너)
- `drizzle/0001_add_env_column.sql` (DB 마이그레이션)
...
**마지막 업데이트**: 2026-02-08 (멀티 환경 지원 완료)  
