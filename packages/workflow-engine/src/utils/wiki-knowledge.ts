...
import { getRepoManager } from './repo-manager';
...
// Table name prefixes that map to manage subdirectories
...
// Max characters to include per document to stay within token budget
const MAX_DOC_CHARS = 6000;
const MAX_TOTAL_CHARS = 24000; // ~8,000 tokens

// ─── Types ───────────────────────────────────────────────────────────────────

type WikiIndex = {
  tables: Map<string, string>; // table name → file path
  domains: Map<string, string>; // domain key → _overview.md path
...
export class WikiKnowledge {
  ...
  constructor(wikiPath?: string) {
    if (wikiPath) {
      ...
      this.wikiPath = repoManager.wikiPath || getEnv('WIKI_LOCAL_PATH', '');
    }
  }
  ...
  /**
   * Build an in-memory index by scanning the wiki directory.
   * Called once lazily, result is cached.
   */
  async buildIndex(): Promise<WikiIndex> {
    ...
    // Scan manage domain directories
  ...
  /**
   * Load the base RTB context (always included).
   */
  ...
  /**
   * Search wiki for context relevant to the given text.
   * Extracts table names and domain keywords, loads matching docs.
   */
  async searchForContext(text: string, maxDocs = 4): Promise<string> {
    ...
    // 2. Extract table names from text
    ...
    // 3. Extract domain keywords and load overviews (if not already covered)
    ...
    // 4. Include relevant query patterns if tables were found
  ...
  /**
   * Get a domain overview document.
   */
  ...
  /**
   * Extract RTB table name patterns from text.
   * Matches: obj_bld_mst, prd_pdm_mst, gokr.buildings, etc.
   */
  extractTableNames(text: string): string[] {
    const names = new Set<string>();

    // manage table pattern: obj_xxx, prd_xxx, etc.
    ...
    // gokr table pattern: gokr.xxx or standalone known gokr tables
  ...
  /**
   * Extract matched domain keys from text using keyword mapping.
   */
  extractDomains(text: string): string[] {
    ...
    // Also detect domains from table name prefixes
    const tableNames = this.extractTableNames(text);
    for (const tableName of tableNames) {
  ...
  /**
   * Extract query sections from queries.md that are relevant to the given tables/domains.
   */
  private async extractRelevantQueries(
    ...
    try {
      ...
      // Domain name mapping for query section matching
      ...
      for (const section of sections.slice(1)) {
        ...
        // Check if section mentions any of the tables
